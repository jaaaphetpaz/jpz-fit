...
export function generateWorkout(args) {
  ...
  if (!listOfMuscles) {
    console.error("List of muscles is undefined or empty:", listOfMuscles);
    return [];
  }
  ...
  if (!SCHEMES[scheme]) {
    console.error("Scheme is undefined:", scheme);
    return [];
  }

  if (!SCHEMES[scheme].ratio) {
    console.error("Scheme ratio is undefined:", SCHEMES[scheme]);
    return [];
  }
